...
const renderPage = (pageId: string) => {
                ...
                <section class="bg-blue-800 text-white py-16 sm:py-24 px-4 relative overflow-hidden">
                    ...
                    <div class="container mx-auto text-center max-w-4xl relative z-10">
                        ...
                        <div class="flex flex-col sm:flex-row justify-center items-center gap-4">
                            <button id="hero-announcements-btn" class="w-full sm:w-auto bg-yellow-400 text-blue-900 font-black px-10 py-4 rounded-2xl shadow-xl hover:bg-yellow-500 hover:scale-105 transition-all text-sm uppercase tracking-widest">Avisos Importantes</button>
                            <button id="hero-school-btn" class="w-full sm:w-auto bg-blue-700 text-white font-bold px-10 py-4 rounded-2xl shadow-lg border border-blue-600 hover:bg-blue-600 transition-all text-sm uppercase tracking-widest">Conheça a Escola</button>
                        </div>
                    </div>
                </section>
                ...
                <section class="container mx-auto px-4 -mt-12 mb-20">
                    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
                        ${['school', 'studies', 'interclasses', 'announcements'].map(p => `
                            <button data-goto="${p}" class="quick-access-btn bg-white p-6 sm:p-10 rounded-3xl shadow-xl card-hover flex flex-col items-center text-center border border-slate-100 group relative overflow-hidden">
    ...
    // Re-attach dynamic button listeners after innerHTML injection
    if (pageId === 'home') {
        const annBtn = document.getElementById('hero-announcements-btn');
        if (annBtn) annBtn.addEventListener('click', () => navigateTo('announcements'));
        
        const schoolBtn = document.getElementById('hero-school-btn');
        if (schoolBtn) schoolBtn.addEventListener('click', () => navigateTo('school'));

        document.querySelectorAll('.quick-access-btn').forEach((btn: any) => {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Setup global listeners (avoiding onclick in HTML)
    ...
    document.querySelectorAll('.nav-link, .mobile-nav-link').forEach((link: any) => {
        link.addEventListener('click', () => navigateTo(link.dataset.page));
    });

    // Footer specific links
    const fAnn = document.getElementById('footer-link-announcements');
    if (fAnn) fAnn.addEventListener('click', () => navigateTo('announcements'));
    
    const fStd = document.getElementById('footer-link-studies');
    if (fStd) fStd.addEventListener('click', () => navigateTo('studies'));

    const fInt = document.getElementById('footer-link-interclasses');
    if (fInt) fInt.addEventListener('click', () => navigateTo('interclasses'));

    // Initial load
    navigateTo('home');
});

// Fix: Adding an empty export to turn this script into a module and avoid global namespace conflicts
export {};
